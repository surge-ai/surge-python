...
def test_save_report_downloads_when_request_returns_ready():
    ...
    sink = io.BytesIO()
    with (
        mock.patch.object(Report, "request", return_value=ready) as mock_request,
        mock.patch.object(Report, "check_status") as mock_check,
        mock.patch("urllib.request.urlopen") as mock_urlopen,
    ):
        mock_urlopen.return_value.__enter__.return_value = io.BytesIO(_gzipped(payload))
...
def test_save_report_polls_check_status_for_returned_job_id():
    ...
    completed = Report(status="COMPLETED", url="https://signed.example/report.gz")
    sink = io.BytesIO()
    with (
        mock.patch.object(Report, "request", return_value=creating) as mock_request,
        mock.patch.object(
            Report, "check_status", side_effect=[in_progress, completed]
        ) as mock_check,
        mock.patch("urllib.request.urlopen") as mock_urlopen,
...
def test_save_report_switches_job_id_on_retrying():
    ...
    with (
        ...
        mock.patch.object(
            Report, "check_status", side_effect=[retrying, in_progress, completed]
        ) as mock_check,
        mock.patch("urllib.request.urlopen") as mock_urlopen,
...
def test_save_report_raises_on_unexpected_status():
    ...
    with (
        ...
        with pytest.raises(ValueError, match="ERROR"):
            Report.save_report("proj-123", "export_json", filepath=io.BytesIO())
...
def test_save_report_times_out_when_job_never_completes():
    ...
    with (
        mock.patch.object(Report, "request", return_value=creating),
        mock.patch.object(Report, "check_status", return_value=in_progress),
        mock.patch("time.sleep"),
