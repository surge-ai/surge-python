...
def test_convert_questions_to_objects():
    ...
    assert questions[0].label == "checkbox"
    ...
    assert questions[1].label == "multiple_choice"
    ...
    assert questions[2].label == "free_response"
    ...
    assert questions[4].text == "Text area for {{url}}"
    assert questions[4].label == "text"
    assert questions[4].required == False
    assert not hasattr(questions[4], "options")

    assert type(questions[5]) == ChatBot
    assert questions[5].text == "Chatbot for {{url}}"
